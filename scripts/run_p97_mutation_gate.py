...
def build_summary(results: list[dict[str, Any]], registries: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "p97-rule-coverage-mutation-gate-v1",
        "generated_at_utc": datetime.now(UTC).isoformat(),
        "claim": "Representative compliance-rule mutation gate passed; remaining rules are explicitly mapped, not claimed complete.",
        "ok": not failed,
        ...
        "boundaries": [
            "No native Office replay was run for P97.",
            "P97 does not change semantic-editable row counts.",
            "Rules marked uncovered_in_p97 are not claimed mutation-covered.",
...
def generator_source_files() -> list[dict[str, Any]]:
    return [
        manifest_file(ROOT / "scripts" / "run_p97_mutation_gate.py"),
        manifest_file(ROOT / "scripts" / "p97_mutation_cases.py"),
    ]
...
def write_outputs(summary: dict[str, Any], out_dir: Path) -> None:
    ...
    manifest = {
        "schema_version": "p97-evidence-manifest-v1",
        "generated_at_utc": datetime.now(UTC).isoformat(),
...
def run_gate(out_dir: Path = EVIDENCE) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="p97-mutation-") as tmp:
